...
async function slotsDisponiveis(profissionalId, data, duracao = DURACAO_PADRAO_MIN) {
  ...
  const livres = GRID_SLOTS.filter(hora => {
    ...
    return !hasConflito(agendamentos, { profissionalId, data, hora, duracao });
  }).slice(0, MAX_SLOTS_LISTADOS);
...
async function criar({ clienteId, profissionalId, data, hora, duracao = DURACAO_PADRAO_MIN }) {
  const [diasBloqueados, agendamentos] = await Promise.all([
    carregarDiasBloqueados(),
    fetchTodosAgendamentos(),
  ]);
  ...
  const agendamento = {
    ...
    status: 'agendado',
    recorrencia: 'nenhuma',
    obs: 'Criado via bot do WhatsApp',
  };
