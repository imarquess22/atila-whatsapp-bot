const { sbGet, sbInsert, sbPatch } = require('./supabase');
const { uid, toMin, hojeSP, horaAgoraSP } = require('./util');
const { GRID_SLOTS, DURACAO_PADRAO_MIN, MAX_SLOTS_LISTADOS, MAX_AGENDAMENTOS_LISTADOS } = require('./constants');
...
async function fetchProfissionaisAtivos() {
  ...
  return linhas.map(l => l.dados).filter(p => p?.status === 'ativo');
}
...
async function fetchTodosAgendamentos() {
  ...
  return linhas.map(l => ({ id: l.id, ...l.dados }));
}

// Procedimentos cadastrados em Configurações → Planos Disponíveis (mesmo array que o index.html usa).
...
async function fetchProcedimentos() {
  ...
  const planos = linhas[0]?.dados?.planos || [];
...
// considerando conflitos, bloqueios (dia inteiro/parcial) e, se for hoje, os horários que já passaram.
async function slotsDisponiveis(profissionalId, data, duracao = DURACAO_PADRAO_MIN) {
  ...
  const { bloqueado, slots } = calcularSlotsDoDia(agendamentos, diasBloqueados, profissionalId, data, duracao);
  return { bloqueado, slots: slots.slice(0, MAX_SLOTS_LISTADOS) };
}
...
async function listarFuturos(clienteId) {
  ...
  const hoje = hojeSP();
  return todos
    .filter(a => a.clienteId === clienteId && a.status !== 'cancelado' && a.data >= hoje)
