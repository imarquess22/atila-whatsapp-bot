...
// Busca a janela de horário de atendimento definida em Calendário → Horário de Atendimento
// (config.horarioInicio/horarioFim). Só restringe o que o BOT oferece pras clientes escolherem —
// não afeta agendamentos criados manualmente no portal (index.html tem seu próprio fluxo de
// salvarAgendamento, completamente à parte deste módulo). Sem horário definido, usa a grade cheia
// (07:00–21:00) como sempre foi.
async function fetchHorarioAtendimento() {
  const cfg = await fetchConfigStudio();
  return { inicio: cfg.horarioInicio || null, fim: cfg.horarioFim || null };
}
...
// `horario` (opcional): { inicio, fim } — janela de atendimento configurada pra oferta ao bot.
function calcularSlotsDoDia(agendamentosExistentes, diasBloqueados, profissionalId, data, duracao, horario = {}) {
  ...
  const livres = GRID_SLOTS.filter(hora => {
    ...
    if (inicio && hora < inicio) return false;
    if (fim && hora >= fim) return false;
...
async function slotsDisponiveis(profissionalId, data, duracao = DURACAO_PADRAO_MIN) {
  const [diasBloqueados, agendamentos, horario] = await Promise.all([
    carregarDiasBloqueados(),
    fetchTodosAgendamentos(),
    fetchHorarioAtendimento(),
  ]);
  return calcularSlotsDoDia(agendamentos, diasBloqueados, profissionalId, data, duracao, horario);
}
...
async function diasComDisponibilidade(profissionalId, ano, mes, duracao) {
  ...
  for (let d = 1; d <= ultimoDia; d++) {
    ...
    const { bloqueado, slots } = calcularSlotsDoDia(agendamentos, diasBloqueados, profissionalId, dataStr, duracao, horario);
...
async function reagendar(id, novaData, novaHora) {
  ...
  // Histórico de remarcações — mesmo formato usado pelo portal (index.html » salvarAgendamento),
  // pra dar pra ver no app de onde e quando cada remarcação veio (ajuda a achar furos na agenda).
  const historico = [...(atual.historico || []), {
    de: { data: atual.data, hora: atual.hora }, para: { data: novaData, hora: novaHora },
    em: new Date().toISOString(), por: 'whatsapp',
  }];
  // 'dados' guarda o objeto inteiro (incluindo o próprio id), igual ao que o index.html salva.
  const atualizado = { ...atual, data: novaData, hora: novaHora, status: 'agendado', historico };
