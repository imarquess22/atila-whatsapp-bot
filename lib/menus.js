...
function listaProcedimentos(procedimentos) {
  ...
  const linhas = procedimentos.map((p, i) => ({
    id: String(i + 1), titulo: p.nome, descricao: `R$ ${(p.valor || 0).toFixed(2)} · ${p.duracaoMin}min`,
  }));
  linhas.push({ id: '0', titulo: '↩️ Voltar ao menu' });
  const texto = `Qual procedimento você quer agendar?\n\n` +
    procedimentos.map((p, i) => `${i + 1}️⃣ ${p.nome} — R$ ${(p.valor || 0).toFixed(2)} (${p.duracaoMin}min)`).join('\n') +
    `\n\nResponda com o número.`;
