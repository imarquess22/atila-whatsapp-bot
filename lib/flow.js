...
const { mesAtualEProximo } = require('./util');
const { DURACAO_PADRAO_MIN, MAX_PROFISSIONAIS_LISTADOS, MAX_PROCEDIMENTOS_LISTADOS, DIAS_POR_PAGINA } = require('./constants');
...
async function tratarEscolherDia(session, texto, proximoStep) {
  ...
  session.context.slotsOferecidos = slots;
  session.step = proximoStep;
  return { resposta: menus.listaSlots(slots, diaEscolhido) };
}
