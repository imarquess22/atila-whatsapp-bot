const { getSession, saveSession, estadoPadrao, comMenuPrincipal, humanTakeoverAtivo } = require('./session');
...
async function tratarMenuPrincipal(session, cliente, texto) {
  switch (texto.trim()) {
    case '1': {
      ...
      if (!profs.length) return { resposta: `No momento não há profissionais disponíveis para agendamento.\n\n${menus.menuPrincipal(cliente.nome)}` };
    ...
    case '3': {
      const futuros = await agenda.listarFuturos(cliente.id);
      if (!futuros.length) return { resposta: `Você não tem agendamentos futuros para remarcar.\n\n${menus.menuPrincipal(cliente.nome)}` };
      const todosProfs = await agenda.fetchTodosProfissionais();
      const porId = Object.fromEntries(todosProfs.map(p => [p.id, p]));
    ...
    case '4': {
      const futuros = await agenda.listarFuturos(cliente.id);
      if (!futuros.length) return { resposta: `Você não tem agendamentos futuros para cancelar.\n\n${menus.menuPrincipal(cliente.nome)}` };
      const todosProfs = await agenda.fetchTodosProfissionais();
      const porId = Object.fromEntries(todosProfs.map(p => [p.id, p]));
      session.context.agendamentosListados = futuros.map(a => ({ id: a.id, data: a.data, hora: a.hora, duracao: a.duracao || DURACAO_PADRAO_MIN, profissionalId: a.profissionalId, profissionalNome: porId[a.profissionalId]?.nome || 'Profissional' }));
...
async function tratarAgendarProfissional(session, texto) {
  ...
  if (ehVoltar(texto)) { const s = comMenuPrincipal(session); return { session: s, resposta: null, voltarMenu: true }; }
...
async function tratarAgendarData(session, texto) {
  if (ehVoltar(texto)) { const s = comMenuPrincipal(session); return { session: s, voltarMenu: true }; }
...
async function tratarAgendarConfirmar(session, cliente, texto) {
  ...
  const s = comMenuPrincipal(session);
  if (!resultado.ok) {
    return { session: s, resposta: `${menus.erroGenerico(resultado.erro)}\n\n${menus.menuPrincipal(cliente.nome)}` };
  }
  await registrarAcesso('criar', { entidade: 'agendamento', descricao: `${cliente.nome} c/ ${profissionalNome} em ${data} ${hora} (via WhatsApp)` });
  return { session: s, resposta: `${menus.agendamentoCriado({ profissionalNome, data, hora })}\n\n${menus.menuPrincipal(cliente.nome)}` };
}
...
async function tratarRemarcarConfirmar(session, cliente, texto) {
  ...
  return { session: s, resposta: `${menus.reagendamentoConfirmado({ profissionalNome, data, hora })}\n\n${menus.menuPrincipal(cliente.nome)}` };
}
...
async function tratarCancelarConfirmar(session, cliente, texto) {
  ...
  return { session: s, resposta: `${menus.cancelamentoConfirmado()}\n\n${menus.menuPrincipal(cliente.nome)}` };
}

// Ponto de entrada único: processa uma mensagem recebida e retorna o texto de resposta
...
async function handleIncoming({ from, msgId, text }) {
  ...
  if (textoLower === 'menu') {
    ...
    session = comMenuPrincipal(session);
    if (!cliente) {
  ...
  if (!cliente) {
    ...
    session = comMenuPrincipal(session);
    return finalizar(`Prazer, ${cliente.nome}! Cadastro feito. ✅\n\n${menus.menuPrincipal(cliente.nome)}`);
  }
